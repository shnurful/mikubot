...
class SongSelect(discord.ui.Select):
    def __init__(self, songMap):
        ...
        super().__init__(
            placeholder="Select a result", max_values=1, min_values=1, options=songMap
        )
...
class SongSelectView(discord.ui.View):
    def __init__(self, songMap):
        ...
        options = [
            ...
            for title, url in songMap.items()
        ]
...
class music(commands.Cog):
    ...
    async def start_dc_timer(self,ctx):
        """start timer to disconnect after 5 minutes of being idle"""
    ...
    def stop_dc_timer(self):
        """stops running dc timer"""
    ...
    async def do_autoplay(self, ctx):
        ...
        if self.loop_on:
            ...
            self.disconnect_timer = self.bot.loop.create_task(self.start_dc_timer(ctx))
            return

    @commands.hybrid_command(
        name="play", description="enter a url or search for a song"
    )
    async def play(self, ctx: commands.Context, query_or_url: str):
        ...
        try:
            ...
            if vc.is_playing():
                loop = asyncio.get_event_loop()
                data = await loop.run_in_executor(
                    None, lambda: self.ytdl.extract_info(query_or_url, download=False)
                )
                qitem = {"title": data["title"], "url": query_or_url}
                self.music_queue.append(qitem)
                await ctx.interaction.followup.send(
                    f"Ok! I added \"{data['title']}\" to the queue!"
                )
                return
    ...
    @commands.hybrid_command(
        name="pause", description="I'll pause what's currently playing"
    )
    ...
    @commands.hybrid_command(
        name="resume", description="I'll resume what's currently playing"
    )
    ...
    @commands.hybrid_command(
        name="stop", description="I'll stop playing the current song"
    )
    ...
    @commands.hybrid_command(
        name="now-playing", description="I'll show what's playing right now"
    )
    ...
    @commands.hybrid_command(
        name="play-next", description="I'll play the next song in queue"
    )
    ...
    @commands.hybrid_command(
        name="remove-from-queue", description="I'll remove this song from queue"
    )
